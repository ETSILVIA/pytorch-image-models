...
class VGNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # self.triple_eye=nn.Linear(channels[-1],64)
        # self.triple_glass=nn.Linear(channels[-1],64)
    ...
    def forward(self, x):
        ...
        triplets_glass = self.glass(x)
        triplets_eye = self.eye(x)
        return triplets_glass,triplets_eye,x,x
...
@export
@blocks.se(partial(nn.Hardswish, inplace=True))
...
def vgnetg_1_0mp_se(pretrained: bool = False, pth: str = None, progress: bool = True, **kwargs: Any):
    kwargs['channels'] = [28, 56, 112, 224, 256]
    kwargs['downsamplings'] = ['blur', 'blur', 'blur', 'blur']
    kwargs['layers'] = [4, 7, 13, 2]
...
if __name__=='__main__':
    ...
    model=vgnetg_1_0mp_se()
    dummy_input = torch.randn(1,1, 3, 128, 128)
