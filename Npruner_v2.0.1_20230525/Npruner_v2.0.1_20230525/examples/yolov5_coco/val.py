...
def run(model=None,
    ...
    training = model is not None and is_training
    ...
    if not training:
        if device.type != 'cpu':
            model(torch.zeros(1, 12, int(imgsz/2), int(imgsz/2)).to(device).type_as(next(model.parameters())))  # run once
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./weight/yolov5s6.pt', help='model.pt path(s)')
