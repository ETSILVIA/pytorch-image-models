...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        self.stage3 = self._make_stage(int(256 * width_multipliers[2]), num_blocks_per_stage[2],
                                       num_se_blocks=int(num_blocks_per_stage[2] // 2) if use_se else 0,use_relu=True)
        self.stage4 = self._make_stage(int(512 * width_multipliers[3]), num_blocks_per_stage[3],
                                       num_se_blocks=num_blocks_per_stage[3] if use_se else 0,use_relu=False)
        # self.activation=nn.LeakyReLU()
        self.gap = nn.AdaptiveAvgPool2d(output_size=1)
        self.glasses = nn.Linear(int(512 * width_multipliers[3]), 3)
    
        self.eyes = nn.Linear(int(512 * width_multipliers[3]), 3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(x.size(0), -1)#调整维度，同理，如果是x.view[参数，-1]就是根据参数来调整列数
        # print(x)
        x1 = self.glasses(x)
        x2 = self.eyes(x)
        # x1=self.Sigmoid(x1)
...
@register_model
def mobileone_s1(pretrained=False,state='train'):
    """ MobileNet V3 """
    if state=='train':
...
@register_model
def mobileone_s2(pretrained=False,state='train'):
    """ MobileNet V3 """
